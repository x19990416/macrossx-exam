...
class QuestionGeneral extends PureComponent {
  ...
  handleSubmit =(data)=>{  
    ...
    if('修改知识点' === this.state.modalProps.title){
  ...
  handleUpdateModalVisible = (flag,record) => {    
    console.log('asdfjal;ksdjf;lkasjdf;lkasdj\t',record) 
    this.setState({
      modalVisible: flag,
      modalProps: {
        ...record,
    ...
    return (
      <Form  layout="inline" onSubmit={this.handleSearch}>
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={8} sm={24}>
            <Form.Item label="知识点名">
                {getFieldDecorator('name')(<Input placeholder="请输入"/>)}
            </Form.Item>
          </Col>
          <Col md={8} sm={24}>
            <Form.Item label="题库名">
              {getFieldDecorator('fieldName')(<Input placeholder="请输入"/>)}
            </Form.Item>
          </Col>
          <Col md={8} sm={24}>
            <span>
  ...
  render() {
    ...
    return (
       ...
       <Card bordered={false}>
        <div className={styles.tableList}>
          ...
          <div className={styles.tableListOperator}>
            <Button icon="plus" type="primary" onClick={()=>this.handleModalVisible(true,{title:'新增知识点'})} >新建</Button>
      ...
      <ModalForm {...this.props.modalBody} modalVisible={this.state.modalVisible} modalProps= {this.state.modalProps} onCancel={()=>this.handleModalVisible(false)} onSubmit={(data)=>{this.handleSubmit(data)}}> </ModalForm>      
    </PageHeaderWrapper> 
  )}
}
